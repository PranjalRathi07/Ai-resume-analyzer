...
const { canCallAI } = require("../utils/aiUtils");
...
exports.uploadAndAnalyze = async (req, res) => {
	try {
		...
		if (!canCallAI(req.userId, "upload", 15000)) {
			return res.status(429).json({ message: "Too many upload requests. Please wait a few seconds." });
		}
		...
		if (recentResume && recentResume.analysisData && recentResume.analysisData.overallScore > 0) {
			...
			return res.status(200).json({
				...
				resumeId: recentResume._id,
				fileUrl: recentResume.fileUrl,
				analysisData: recentResume.analysisData,
			});
		}
		...
		if (
			...
			return res.status(429).json({ 
				message: "AI Rate Limit Reached. Google's free tier allows 15 requests per minute and limits daily usage. Please wait a few minutes and try again." 
			});
		}
		res.status(500).json({
